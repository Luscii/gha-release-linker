...
import {
  getLinearIssueFromPrUrl,
  moveIssueToDoneIfReady
} from './linear_issue.util.js'
...
const ENABLED_LINEAR_PREFIXES: string | undefined = core.getInput(
  'ENABLED_LINEAR_PREFIXES'
)
...
export async function processRelease(): Promise<void> {
  ...
  if (!LINEAR_API_KEY) {
    throw new Error(
      'Error: LINEAR_API_KEY environment variable is not set. Please configure it in your CI/CD pipeline secrets.'
    )
  }
  if (!GITHUB_TOKEN) {
    throw new Error(
      'Error: GITHUB_TOKEN environment variable is not set. Please configure it in your CI/CD pipeline secrets.'
    )
  }
  if (!GITHUB_ORG) {
    throw new Error(
      'Error: GITHUB_ORG environment variable is not set. Please configure it in your environment or .env file.'
    )
  }
  if (!GITHUB_REPO) {
    throw new Error(
      'Error: GITHUB_REPO environment variable is not set. Please configure it in your environment or .env file.'
    )
  }
  ...
  for (const prUrl of prUrls) {
    ...
    if (linearIssue && !updatedIssues.has(linearIssue.id)) {
      if (!isIdentifierEnabled(linearIssue.identifier)) {
        console.log(
          `Skipping PR, disabled project prefix in: ${linearIssue?.identifier}`
        )
        continue
      }
...
const enabledPrefixes: string[] = (ENABLED_LINEAR_PREFIXES || '')
  ...
  .filter(Boolean)
  .map((s) =>
    s.toUpperCase().endsWith('-') ? s.toUpperCase() : s.toUpperCase() + '-'
  )
