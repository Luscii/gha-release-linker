...
export async function getPullRequestUrlsForRelease(
  ...
  if (!GITHUB_ORG || !GITHUB_REPO) {
    console.warn(
      'GitHub org/repo environment variables are not set; returning empty PR list.'
    )
    return []
  }
  ...
  try {
    ...
    const fullUrlRegex = new RegExp(
      `https://github.com/${GITHUB_ORG}/${GITHUB_REPO}/pull/(\\d+)`,
      'g'
    )
    ...
    while ((match = prNumberRegex.exec(releaseBody)) !== null) {
      ...
      prUrls.push(
        `https://github.com/${GITHUB_ORG}/${GITHUB_REPO}/pull/${prNumber}`
      )
    }
    ...
    console.error(
      `Error fetching release ${versionName} from GitHub:`,
      error?.message || error
    )
    if (error?.status === 404) {
      console.error(
        `Release tag '${versionName}' not found in ${GITHUB_ORG}/${GITHUB_REPO}.`
      )
